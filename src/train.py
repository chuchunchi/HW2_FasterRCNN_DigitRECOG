...
def train_one_epoch_sgd(model, data_loader, optimizer, device, epoch,
                        print_freq=100):
...
def train_one_epoch(model, data_loader, optimizer, lr_scheduler, dev, epoch):
    ...
    for i, (images, targets) in enumerate(data_loader):
        ...
        if i % 100 == 0:
            print(
                f'Epoch: [{epoch}][{i}/{len(data_loader)}] {metric_logger} \
...
def train(cfg, checkpoint_path=None):
    ...
    if cfg.lr == "step":
        ...
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(
            ...
            total_steps=cfg.num_epochs * len(train_loader) + 5,  # extra buffer
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if (torch.cuda.is_available() and
                ...
                0.95 * torch.cuda.get_device_properties(0).total_memory):
