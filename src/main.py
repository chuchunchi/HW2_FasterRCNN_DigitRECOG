...
class DigitDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Open image and convert to RGB, close it after processing
        ...
        if self.coco is not None and img_file in self.file_to_id:
            ...
            img_id = int(os.path.splitext(img_file)[0])  # id = filename
            target = {
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Digit Recognition with Faster R-CNN')
    parser.add_argument(
        '--mode', type=str, choices=['train', 'eval', 'inference'],
        default='train', help='Running mode'
    )
    parser.add_argument('--checkpoint', type=str,
                        default=None, help='Model checkpoint path')
